...
AfrimInputMethodEngine::listInputMethods()
{
  ...
  entries.push_back(
    std::move(fcitx::InputMethodEntry("afrim", "Afrim", "af", "afrim")
                .setLabel("AF")
...
void
AfrimInputMethodEngine::activate(const fcitx::InputMethodEntry&,
...
void
AfrimInputMethodEngine::deactivate(const fcitx::InputMethodEntry&,
...
void
AfrimInputMethodEngine::reset(const fcitx::InputMethodEntry&,
                              fcitx::InputContextEvent& event)
{
  if (engine_) {
    afrim_engine_reset(engine_);
...
void
AfrimInputMethodEngine::keyEvent(const fcitx::InputMethodEntry&,
                                 fcitx::KeyEvent& event)
{
  ...
  char* cmds = afrim_engine_process_key(engine_,
                                        sym,
                                        modState,
                                        /*is_pressed=*/1,
                                        keyStr.c_str());
...
void
AfrimInputMethodEngine::applyCommands(fcitx::InputContext* ic, const char* cmds)
{
  if (!cmds || !*cmds)
    return;
...
void
AfrimInputMethodEngine::updateUI(fcitx::InputContext* ic)
{
  if (!engine_)
    return;
...
void
AfrimInputMethodEngine::clearUI(fcitx::InputContext* ic)
{
  output.clear();
  ic->inputPanel().reset();
  ic->updatePreedit();
